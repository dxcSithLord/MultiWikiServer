...
if npx --no-install spectral --version >/dev/null 2>&1; then
  ...
  warn "Spectral not installed; skipping lint. Install: npm i -D @stoplight/spectral-cli @stoplight/spectral-owasp-ruleset"
fi
...
# Drift guard: surface any regex `path:` declarations so a new one can't slip past the
# pinned table above unnoticed.
regex_src=$(grep -rEn 'path:[[:space:]]*(/\^|new RegExp)' "$SRC" --include=*.ts | grep -vc '/__tests__/')
info "regex path declarations in source: $regex_src (pinned table covers the known set;"
info "  if this grew, add the new regex route to openapi.yaml and REGEX_DOC_PATHS above)"
