...
SendStream.prototype.send = function send(path, stat) {
  ...
  if(options.prefix) len += options.prefix.byteLength;
  if(options.suffix) len += options.suffix.byteLength;
...
SendStream.prototype.stream = function stream(path, options) {
  ...
  if(options.prefix) res.write(options.prefix);
  stream.pipe(res, { end: !options.suffix });
  ...
  stream.on('end', function onend() {
    if(options.suffix) res.end(options.suffix);
    self.emit('end')
  })
}
