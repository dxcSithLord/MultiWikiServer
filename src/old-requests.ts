...
class Server {
  constructor(options) {
    ...
    this.protocol = "http";
    var tlsKeyFilepath = this.get("tls-key"), tlsCertFilepath = this.get("tls-cert"), tlsPassphrase = this.get("tls-passphrase");
  ...
  addAuthenticator(AuthenticatorClass) {
    ...
    var authenticator = new AuthenticatorClass(this), result = authenticator.init();
  ...
  */
  ...
  requestHandler(request, response, options) {
    options = options || {};
  ...
  */
  listen(port, host, prefix, options) {
    var self = this;
    ...
    server.on("listening", function() {
      ...
      var address = server.address(), url = self.protocol + "://" + (address.family === "IPv6" ? "[" + address.address + "]" : address.address) + ":" + address.port + prefix;
